...
- Even after the tools existed, the name kept losing: OMP ships built-in `checkpoint`/`rewind` context tools, so models grabbed those instead (`goal=` in, nothing written to disk). The whole surface is renamed to `/snapshot`: skill `snapshot`, tools `snapshot_write` + `changelog_update`, and the built-in pair is now blocked at the `tool_call` hook with a redirect message, so a confused model self-corrects instead of silently parking context. `checkpoint_prepare` is removed (redundant — `snapshot_write` resolves paths internally); storage stays at `.agents/checkpoints/` so existing snapshot files and session dedupe keep working.
